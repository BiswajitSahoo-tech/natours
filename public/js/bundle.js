      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)('http://127.0.0.1:3000/api/v1/booking/checkout-session/' + tourId);

          case 3:
            session = _context.sent;
            console.log(session); // //2) Create checkout from + charge the credit card

            window.location.replace(session.data.session.url);
            console.log(session.data.session.url); // await stripe.redirectToCheckout({
